...
class BotPlugin : Plugin() {
    ...
    @PluginMethod
    fun ladderUpdate(call: PluginCall) {
        val d = ladderBot.settings
        ladderBot.update(
            LadderPlan.Settings(
                enabled = call.getBoolean("enabled") ?: d.enabled,
                bankUsd = call.getDouble("bankUsd") ?: d.bankUsd,
                shares = call.getDouble("shares") ?: d.shares,
                everySec = call.getInt("everySec")?.toLong() ?: d.everySec,
                firstAtSec = call.getInt("firstAtSec")?.toLong() ?: d.firstAtSec,
    ...
    @PluginMethod
    fun ladderReset(call: PluginCall) {
        ladderBot.resetBank()
        call.resolve()
    }

    @PluginMethod
    fun ladderState(call: PluginCall) {
        val bot = ladderBot
        val t = bot.totals
        val round = bot.round
        ...
        call.resolve(
            JSObject()
                ...
                .put("everySec", bot.settings.everySec)
                .put("firstAtSec", bot.settings.firstAtSec)
                ...
                .put("round", round?.let {
                    JSObject()
                        .put("windowStart", it.windowStart)
                        .put("side", it.lastSide)
                        .put("ask", it.lastAsk)
                        .put("rung", it.lastRung)
                        .put("step", it.step)
                        .put("note", it.note)
                        .put("lots", lots)
                }),

    ...
    <div className="card tight">
      ...
      <div className="counterrule muted">
        ...
        {state.inside ? (
          ...
          максимума, брать нечего — тогда ставит лимитку по 54¢ и ждёт минуту;
          не налили — снимает, чтобы деньги не висели до конца окна. За 10 с до начала перечитывает картину и снимает заявку, если
          сторона поменялась. Уже купленное продаётся
          только лесенкой. Лесенка продаж — потолок выхода: как только цена
          дошла до ступени, продаёт там, а не ждёт, что дадут дороже. Раньше
          выйти можно только по трём причинам, и каждая из них — не «подержим
          ещё», а «забираем сейчас». Пока минутные свечи закрываются в нашу
          сторону — не трогает ничего: ход идёт. Первая свеча в обратную —
          это остановка хода, и если позиция уже в плюсе минимум на 15% после
          комиссии, забирает прямо там. Второе: цена дошла до уровня, ради
          которого бралась сделка. Третье: цена входа удвоилась — сторона за
          треть продаётся на двух третях, не дожидаясь ступеньки.{' '}
          Сторону задаёт только линия: если минутная линия говорит «вбок» —
          направления нет, окно пропускается с «нет линии»; пятиминутная
          возражает лишь тогда, когда сама называет направление. Уровни
          сторону больше не выбирают — отбоя как причины входа нет.
          Из уровней вход останавливают только круглые пятьсот — 88 000,
          88 500, 89 000 — если окно открывается ближе{' '}
          {Math.round(dial.roundBand)}$ к одному из них: стакан там стоит
          всегда, что бы ни говорил график. И хвост последней пятиминутки:
          если на нашей стороне он занял больше трети её размаха — ход туда
          уже сходил и его вернули внутри той же свечи, вход отменяется; если
          хвоста нет — свеча закрылась там, куда дошла, и вход идёт сразу.
          Уровни всё ещё считаются — за целые сутки, раз в минуту, только с
          добавлением новых, так что линии не прыгают, — но нужны они теперь
          для выхода и для графика, а не для того, чтобы решать, куда входить.
          Держит до уровня: как только цена дошла до стены впереди, продаёт
          прямо там и не ждёт ступени — от цены, где рынок уже разворачивался,
          он обычно разворачивается снова. Только в плюс: позиция в минусе у
          стены — это ошибка в стороне, и платить за неё спред нет смысла, расчёт
          решит через пару минут. Всё, что наторгует, ниже по окнам.
          </>
        )}
      </div>
      )}
